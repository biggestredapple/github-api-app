...
import { IRepo, InitSearch, ResponseGenerator } from "types";
...
export function* getUserSaga(actions: PayloadAction<InitSearch>) {
  try {
    yield put(AppActions.load.beginLoad());
    const result: ResponseGenerator = yield call(
      async () => await api().get(`search/users?q=${actions.payload.name}`)
    );
    if (result.data) {
      yield put(
